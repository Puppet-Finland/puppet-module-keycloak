...
Puppet::Type.type(:keycloak_role_attachment).provide(:kcadm, parent: Puppet::Provider::KeycloakAPI) do
  desc ''
  ...
  def realm_roles
    ...
    output = kcadm('get-roles', '', resource[:realm], nil, nil, false, opt => resource[:name])
    begin
